...
import { DOKUMENT_BUCKET } from "@/lib/constants";
import { registerDokument } from "@/app/actions";
import { useDict } from "@/lib/i18n/context";

export function DokumentUpload() {
  const dict = useDict();
  const t = dict.dokumente.upload;
  ...
  async function handleFiles(files: FileList) {
    ...
    for (const [i, file] of liste.entries()) {
      ...
      try {
        await registerDokument({
          storage_path: path,
          dateiname: file.name,
          titel: file.name.replace(/\.pdf$/i, "") || file.name,
        });
        ok += 1;
      } catch (e) {
